const fsSource = `
    ...
    void main() {
        if (vShaderType == 0.0){
            ...
            highp vec3 vLightingPhong = (vAmbientLight * 0.1) + (vDirectionalLight * max(dot(vNPhong.xyz, vL), 0.0) * 1.0) + (vDirectionalLight * pow(max(dot(vH, vNPhong.xyz), 0.0), 20.0) * 10.0 * 1.0);
