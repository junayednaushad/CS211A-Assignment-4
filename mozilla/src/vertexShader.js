const vsSource = `
    ...
    void main() {
        ...
        gl_Position = uProjectionMatrix * vFragPos;
        vColor = aVertexColor;
        ...
        if (aShaderType == 1.0) {
            ...
            highp vec3 vV = normalize(vec3(0.0, 0.0, -20.0));
            highp vec4 vN = uNormalMatrix * vec4(aVertexNormal, 1.0);
            highp vec3 vH = normalize(vL + vV);
            vLighting = (vAmbientLight * 0.1) + (vDirectionalLight * max(dot(vN.xyz, vL), 0.0) * 1.0) + (vDirectionalLight * pow(max(dot(vH, vN.xyz), 0.0), 20.0) * 10.0 * 1.0);
        }
        if (aShaderType == 2.0) {
